...
export default function MediaGallery({ photos = [], emptyStateMessage = "Aún no hay fotos.", blurImage = false, onUnlock, participant, onLike, onCommentAdded }) {
            ...
            {masonryColumns.map((column, colIdx) => (
              <div key={colIdx} className="flex-1 flex flex-col gap-1 min-w-0">
                {column.map((photo, idx) => (
                  <PhotoCard key={photo.id || idx} photo={photo} blurImage={blurImage} onUnlock={onUnlock} viewMode={viewMode} participant={participant} onLike={onLike} onCommentAdded={onCommentAdded} />
                ))}
              </div>
            ))}
          </div>
